...
public class Main {
    ...
    public static String csvToJson(String[] videos) {
        VideoData[] videoDataResultList = new VideoData[videos.length];
        int increment = 0;
        for(String video : videos) {
            videoDataResultList[increment] = new VideoData(video);
            increment++;
        }

        return VideoData.converterCsvToJson(videoDataResultList);
    }
}